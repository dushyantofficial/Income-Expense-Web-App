<main class="app-content">
    <div class="app-title">
        ...
        <div>
            <h1><i class="fa fa-th-list"></i> @lang('langs.expenses')</h1>
        </div>
 ...
 <a class="btn btn-primary" href="{{ route('expenses.create') }}" style="float: right">
                + @lang('langs.add')
            </a>
                ...
                <div class="box-tools">
                    <form method="GET" action="">
                        <div class="input-group input-group-sm float-right" style="width: 200px;">
                            ...
                            <a href="{{route('expenses.index')}}" class="btn btn-default btn-flat"><span
                ...
                <div class="tile-body" id="my_report">
                    ...
                    <table class="table table-hover table-bordered" id="sampleTable">
                        ...
                        <tr class="sortableTable__header">
                            ...
                            <th class="value" data-col="amount">@lang('langs.expenses_amount') </th>
                        ...
                        @foreach ($expenses as $expense)
                            <tr data-id="{{ $expense->id }}">
                                ...
                                <td data-col="name">{{ $expense->users->name }}</td>
                                <td data-col="accout_name">{{ $expense->account->name }}</td>
                                <td data-col="categories_name">{!! $expense->category->icon !!}
                                    ({{$expense->category->name}})
                                </td>
                                <td data-col="date">{{ $expense->date }}</td>
                                <td data-col="amount"
                                    style="color: red">{{ get_rupee_currency($expense->amount) }}</td>
                                <td data-col="remarks">{{ $expense->remark }}</td>
                    ...
                    <div class="page1">
                        {!! $expenses->appends(request()->all())->render('pagination::bootstrap-4') !!}
                    </div>
                </div>
            </div>
        </div>
