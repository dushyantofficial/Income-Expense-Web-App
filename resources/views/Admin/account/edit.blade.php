...
@section('content')
    ...
    <main class="app-content">
        <div class="app-title">
            <div>
                <h1><i class="fa fa-edit"></i> @lang('langs.bank_update')</h1>
            </div>
        </div>
        @php
                    ...
                    <div class="row">
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input class="form-control @error('user_id') is-invalid @enderror" type="text"
                                       name="name" value="{{$accounts->name}}"
                                ...
                                <select class="form-control @error('user_id') is-invalid @enderror" name="user_id">
                                    @if($user->role == config('constants.ROLE.ADMIN'))
                                        ...
                                        @foreach($users as $user)
                                            <option
                                                value="{{$user->id}}" @if(isset($accounts)){{ $accounts->user_id == $user->id  ? 'selected' : ''}} @endif>{{$user->name}}</option>
                                        @endforeach
                                    @else
                                        <option
                                            value="{{$user->id}}" @if(isset($accounts)){{ $accounts->user_id == $user->id  ? 'selected' : ''}} @endif>{{$user->name}}</option>
                        ...
                        <div class="col-6">
                            <div class="form-group">
                                ...
                                <input type="radio" id="html" name="type"
                                       value="cash" {{old('type')=="cash" ? 'checked='.'"checked"' : '' }} @if(isset($accounts)) {{ ($accounts->type=="cash")? "checked" : "" }} @endif>
                                <label for="female">@lang('langs.bank')</label>
                                <input type="radio" id="css" name="type"
                                       value="bank" {{old('type')=="bank" ? 'checked='.'"checked"' : '' }} @if(isset($accounts)) {{ ($accounts->type=="bank")? "checked" : "" }} @endif>
                        ...
                        <div class="col-6">
                            <div class="form-group">type
                                ...
                                <input class="form-control @error('amount') is-invalid @enderror" type="number"
                                       name="amount" min="1" value="{{$accounts->amount}}"
                        ...
                        <button type="submit" class="btn btn-primary">@lang('langs.update')</button>&nbsp;&nbsp;
