<main class="app-content">
    <div class="app-title">
        ...
        <div>
            <h1><i class="fa fa-th-list"></i> @lang('langs.transfer')</h1>
        </div>
 ...
 <a class="btn btn-primary" href="{{ route('transfer.create') }}" style="float: right">
                + @lang('langs.add')
            </a>
                ...
                <div class="box-tools">
                    <form method="GET" action="">
                        <div class="input-group input-group-sm float-right" style="width: 200px;">
                            ...
                            <a href="{{route('transfer.index')}}" class="btn btn-default btn-flat"><span
                ...
                <div class="tile-body" id="my_report">
                    <table class="table table-hover table-bordered" id="sampleTable">
                        ...
                        <tr class="sortableTable__header">
                            <th class="value" data-col="no">@lang('langs.transfer_no')</th>
                            <th class="value" data-col="name">@lang('langs.user_name') </th>
                            <th class="value" data-col="from_user">@lang('langs.transfer_from_account') </th>
                            <th class="value" data-col="to_user">@lang('langs.transfer_to_account') </th>
                            <th class="value" data-col="date">@lang('langs.incomes_date') </th>
                            <th class="value" data-col="amount">@lang('langs.transfer_amount') </th>
                            <th class="value" data-col="remark">@lang('langs.incomes_remarks') </th>
                        ...
                        @foreach ($transfers as $transfer)
                            <tr data-id="{{ $transfer->id }}">
                                ...
                                <td data-col="name">{{ $transfer->users->name }}</td>
                                <td data-col="from_user">{{ $transfer->from_user->name }}</td>
                                <td data-col="to_user">{{$transfer->to_user->name}} </td>
                                <td data-col="date">{{ $transfer->date }}</td>
                                <td data-col="amount" style="color: darkred">{{ get_rupee_currency($transfer->amount) }}</td>
                                <td data-col="remark">{{ $transfer->remark }}</td>
                    ...
                    <div class="page1">
                        {!! $transfers->appends(request()->all())->render('pagination::bootstrap-4') !!}
                    </div>
                </div>
            </div>
        </div>
